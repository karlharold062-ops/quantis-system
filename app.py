...
class QuantisFinal:  
    ...
    def run_strategy(self):  
        ...
        try:  
            ...
            whale_filter = self.get_whale_filter()  
            news_filter = self.get_cryptopanic_filter()  
            ...
            for symbol in SYMBOLS:  
                ...
                # Décision principale : direction technique  
                technical_signal = data_1d['direction']  
                orderbook_signal = self.analyze_order_book(symbol)  

                # --- APPLICATION DES FILTRES ---  
                if technical_signal == "bullish":  
                    if whale_filter == "bearish" or news_filter == "bearish": continue  # bloque LONG  
                    if orderbook_signal=="buy": self.enter_trade(symbol, data_1d, "LONG")  
                elif technical_signal == "bearish":  
                    if whale_filter == "bullish" or news_filter == "bullish": continue  # bloque SHORT  
                    if orderbook_signal=="sell": self.enter_trade(symbol, data_1d, "SHORT")  
        except Exception as e:  
            print(f"Erreur : {e}")  
    ...
    def enter_trade(self, symbol, data, side):  
        ...
        # Ordre limite réel  
        try:  
            amount = 0.01  # ajuster selon capital  
            if side=="LONG":  
