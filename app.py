...
class QuantisFinal:
    ...
    def enter_trade(self, symbol, data, side):
        try:
            ...
            self.active_trades[symbol] = {
                ...
                "ts": data['atr'] * 1.5,  # Bouclier ATR élargi x1.5
                "partial_done": False, 
    ...
    def manage_active_trade(self, symbol):
        ...
        # Trailing ATR lock (bouclier élargi 1.5)
        atr_trail = data_now['atr'] * 1.5
        ...
        # Partial exit à 1% : vendre moitié et placer SL de l'autre moitié au prix d'entrée
        if pnl >= 1.0 and not trade["partial_done"]:
            self.send_to_wunder(symbol, "partial_exit", price, trade["tp"], trade["sl"], trade["ts"])
            trade["sl"] = trade['entry']  # SL de la moitié restante au prix d'entrée
            trade["partial_done"] = True
            trade["be_protected"] = True
            self.send_notif(f"💰 PROFIT PARTIEL ({symbol}) → +1% encaissé. SL de la moitié restante placé au prix d'entrée (BE).")
            return
