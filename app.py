...
class QuantisFinal:  
    ...
    def get_whale_filter(self):  
        try:  
            ...
            if "transactions" in resp and len(resp["transactions"]) > 0:  
                ...
                return "bearish" if total_inflow > total_outflow else "bullish"  
            return "neutral"  
        except: return "neutral"  

    def get_cryptopanic_filter(self):  
        try:  
            url = f"https://cryptopanic.com/api/v1/posts/?auth_token={CRYPTOPANIC_API}&public=true"  
            resp = requests.get(url, timeout=5).json()  
            bullish = sum(1 for post in resp.get("results",[]) if "bull" in post['title'].lower())  
            bearish = sum(1 for post in resp.get("results",[]) if "bear" in post['title'].lower())  
    ...
    def run_strategy(self):  
        ...
        try:  
            ...
            whale_f = self.get_whale_filter()  
            news_f = self.get_cryptopanic_filter()  
            ...
            for symbol in SYMBOLS:  
                ...
                tech_sig = data_1d['direction']  
                ob_sig = self.analyze_order_book(symbol)  

                if tech_sig == "bullish":  
                    if whale_f == "bearish" or news_f == "bearish": continue  
                    if ob_sig == "buy": self.enter_trade(symbol, data_1d, "LONG")  
                elif tech_sig == "bearish":  
                    if whale_f == "bullish" or news_f == "bullish": continue  
                    if ob_sig == "sell": self.enter_trade(symbol, data_1d, "SHORT")  
        except Exception as e:  
    ...
    def enter_trade(self, symbol, data, side):  
        ...
        # SL SERRÉ À 0.7x ATR POUR LEVIER X25 ET COMPOUND
        sl = entry - data['atr']*0.7 if side=="LONG" else entry + data['atr']*0.7  
        
        self.active_trades[symbol] = {
            "dir": side, "entry": entry, "tp": tp, "sl": sl, 
            "ts": data['atr']*0.5, "partial_done": False, "be_protected": False
        }  

        try:  
            amount = 0.01 # WunderTrading gérera la taille réelle
            if side=="LONG": self.exchange.create_limit_buy_order(symbol, amount, entry)  
            else: self.exchange.create_limit_sell_order(symbol, amount, entry)  
        except: pass  

        self.send_notif(f"🎯 ORDRE {side} ({symbol})\nEntrée : {entry}\nSL (0.7 ATR) : {round(sl,2)}")  
    ...
    def exit_trade(self, symbol):  
        ...
        if self.check_flash_crash(symbol) or self.check_trend_guard(symbol):  
            ...
            self.send_notif(f"🚨 SORTIE SÉCURITÉ ({symbol})")  
